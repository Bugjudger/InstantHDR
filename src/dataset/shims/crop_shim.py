...
#         }
def apply_crop_shim_to_views(views: AnyViews, shape: tuple[int, int], intr_aug: bool = False) -> AnyViews:
    # 如果有 hdr_image，concat 后一起 crop，再拆开
    if views["hdr_image"].mean() > 0:
        combined = torch.cat([views["image"], views["hdr_image"]], dim=-3)  # [V, 6, H, W]
        if "depth" in views.keys():
            combined, intrinsics, depths = rescale_and_crop(combined, views["intrinsics"], shape, depths=views["depth"], intr_aug=intr_aug)
            return {
                **views,
                "image": combined[:, :3],
                "hdr_image": combined[:, 3:],
                "intrinsics": intrinsics,
                "depth": depths,
            }
        else:
            combined, intrinsics = rescale_and_crop(combined, views["intrinsics"], shape, intr_aug)
            return {
                **views,
                "image": combined[:, :3],
                "hdr_image": combined[:, 3:],
