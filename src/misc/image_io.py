...
def save_video(tensor, save_path, fps=15):
    """
...
def save_interpolated_video(
    pred_extrinsics, pred_intrinsics, b, h, w, gaussians, save_path, decoder_func, target_exposure=None, t=10, interpolated_output=None,
):
    ...
    # Include the final input camera before concatenating the trajectory.
    ...
    if interpolated_output is None:
        interpolated_output = decoder_func.forward(
            ...
            (h, w),
            # output_rad=False if target_exposure is None else True,
            # target_exposure = [torch.tensor([float(target_exposure)]) for _ in range(num_frames)] if target_exposure is not None else []
            output_rad=True,
            target_exposure=[torch.tensor([float(target_exposure)], device=pred_all_extrinsic.device)
                             for _ in range(num_frames)] if target_exposure is not None else [],
        )
    else:
        interpolated_output = decoder_func.rerender(
            gaussians,
            interpolated_output,
            target_exposure = [torch.tensor([float(target_exposure)]) for _ in range(num_frames)] if target_exposure is not None else []
            # target_exposure = [torch.tensor([2**v]) for v in np.concatenate([np.linspace(-4.3219280949, -0.5849625007, num_frames // 2 + 1), np.linspace(-0.5849625007, -4.3219280949, num_frames - num_frames // 2)[1:]])]
        )
    ...
    depth_norm = (depth - depth[::num_views].quantile(0.01)) / (
        (depth[::num_views].quantile(0.99) - depth[::num_views].quantile(0.01)).clamp(min=1e-6)
    )
    ...
    return os.path.join(save_path, f"rgb.mp4"), os.path.join(save_path, f"depth.mp4"), interpolated_output
