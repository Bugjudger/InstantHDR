...
def export_ply(
    ...
    if is_hdr:
        # Bake a tone-mapped DC preview into standard 3DGS SH coefficients.
        sh_c0 = 0.28209479177387814
        radiance = torch.exp(f_dc * sh_c0)
        radiance = radiance / torch.quantile(radiance, 0.88).clamp(min=1e-6)
        rgb = torch.log(radiance.clamp(0, 1) * 5000 + 1) / np.log(5001)
        f_dc = (rgb - 0.5) / sh_c0
