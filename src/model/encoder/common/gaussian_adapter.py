...
from ...types import Gaussians, tonemapping
...
class GaussianAdapter(nn.Module):
    ...
    @property
    def d_sh(self) -> int:
        return (self.cfg.sh_degree + 1) ** 2 # each SH_RGB dim

    @property
    def d_in(self) -> int:
        return 7 + 3 * self.d_sh # scale 3; rotation 4; 3 x RGB 
...
class UnifiedGaussianAdapter(GaussianAdapter):
    def forward(
        ...
        if raw_gaussians.shape[-1] == 86:
            scales, rotations, sh, extra_features = raw_gaussians.split((3, 4, 3 * self.d_sh, 4), dim=-1)
        else:
            scales, rotations, sh = raw_gaussians.split((3, 4, 3 * self.d_sh), dim=-1)
            extra_features = None
        # scales, rotations, sh = raw_gaussians.split((3, 4, 3 * self.d_sh), dim=-1)
        ...
        sh = rearrange(sh, "... (xyz d_sh) -> ... xyz d_sh", xyz=3) # torch.Size([1, 764543, 75]) -> torch.Size([1, 764543, 3, 25])
        sh = sh.broadcast_to((*opacities.shape, 3, self.d_sh)) * self.sh_mask # 
        # print(scales.max())
        covariances = build_covariance(scales, rotations) # torch.Size([1, 764543, 3, 3])
        ...
        return Gaussians(
            ...
            extra_features=extra_features.float() if extra_features is not None else None,
        )
