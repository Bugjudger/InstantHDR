...
from src.model.encoder.heads.vggt_dpt_gs_head import VGGT_DPT_GS_Head, HDR_Head, HyperNetwork
# from src.model.encoder.heads.vggt_dpt_gs_head import VGGT_DPT_GS_Head, VGGT_DPT_HDRGS_Head
...
from ..types import tonemapping

inf = float("inf")

def install_qk_norm_hooks(model):
    model._qk_norm_cache = {}
    handles = []

    for i, blk in enumerate(model.aggregator.global_blocks):
        attn = blk.attn

        # 跳过没有norm的
        if not hasattr(attn, "q_norm") or not hasattr(attn, "k_norm"):
            print(f"[skip] block {i}: no q_norm/k_norm")
            continue

        def q_hook(mod, inp, out, block_id=i):
            model._qk_norm_cache.setdefault(block_id, {})
            model._qk_norm_cache[block_id]["q"] = out.detach()

        def k_hook(mod, inp, out, block_id=i):
            model._qk_norm_cache.setdefault(block_id, {})
            model._qk_norm_cache[block_id]["k"] = out.detach()

        handles.append(attn.q_norm.register_forward_hook(q_hook))
...
class EncoderAnySplat(Encoder[EncoderAnySplatCfg]):
    ...
    def __init__(self, cfg: EncoderAnySplatCfg) -> None:
        super().__init__(cfg)
        model_full = VGGT()  # Weights are loaded explicitly by the caller.
        # model_full = VGGT()
        ...
        # HDR Network
        self.hdr_head = HDR_Head(self.gaussian_param_head.input_merger)
        self.hdr_conv = copy.deepcopy(self.gaussian_param_head.scratch.output_conv2)
        self.generate_tonemapper = HyperNetwork()
        # self.tonemapping_param = nn.Parameter(Tonemapping(hidden=64, feat_crf_dim=4).export_params())

        for param in self.hdr_head.guide_encoder.parameters():
            param.requires_grad = False
        for param in self.hdr_conv.parameters():
            param.requires_grad = True
        
    
    def map_pdf_to_opacity( # 让 opacity 在训练早期“没什么话语权”
        self,
    ...
    def forward(
        self,
        image: torch.Tensor,
        exposure: list[torch.Tensor] = None,
        global_step: int = 0,
        ...
        with torch.amp.autocast("cuda", enabled=True, dtype=torch.bfloat16):
            ...
            handles = install_qk_norm_hooks(self)   # 或 install_rope_qk_hooks(model)
        ...
        # exposure = [torch.tensor([2.0], device=image.device, dtype=image.dtype) for _ in range(v)]
        ...
        out = self.gaussian_param_head(
            ...
            image_size=(h, w),
            exposure=exposure,
        ) # torch.Size([1, 5, 84, 448, 448])
        # change to -> torch.Size([1, 5, 128, 448, 448])

        if exposure != None:
            # lr_lograd_embed, hr_img_embed = self.hdr_head(
            #     image,
            #     exposure,
            #     self._qk_norm_cache[14]["q"],
            #     self._qk_norm_cache[14]["q"],
            # ) # torch.Size([1, 5, 128, 448, 448])
            # torch.cuda.empty_cache()
            # hr_lograd_embed = self.upsample(lr_lograd_embed, hr_img_embed)
            # out = out + hr_lograd_embed
            # del hr_lograd_embed, lr_lograd_embed, hr_img_embed

            # hr_rad_embed, mid_logt, hr_img_embed, rel_logt = self.hdr_head(
            #     image,
            #     exposure,
            #     self._qk_norm_cache[14]["q"],
            #     self._qk_norm_cache[14]["q"],
            # ) # torch.Size([1*5, 128, 448, 448])
            # torch.cuda.empty_cache()
            # out = torch.concat([self.hdr_conv(out + hr_rad_embed), self.local_feat_conv(hr_rad_embed)], dim=-3)
            # # tonemapping_param = self.generate_tonemapper(out.view(b, v, *out.shape[1:]).detach(), image, exposure)
            # # tonemapping_param = self.generate_tonemapper(out.view(b, v, *out.shape[1:]).detach(), hr_img_embed.detach(), rel_logt.detach())
            # scene_params = self.generate_tonemapper(hr_img_embed.view(b,v,*hr_img_embed.shape[1:]).detach(), rel_logt, hr_rad_embed.view(b,v,*hr_rad_embed.shape[1:]).detach())
            # tonemapping_param = self.tonemapping_param
            # del hr_rad_embed, hr_img_embed
            # out = out.view(b, v, *out.shape[1:])

            scene_feat, sensitivity, mid_logt, hr_guide, rel_logt = self.hdr_head(
                image,
                exposure,
                self._qk_norm_cache[14]["q"],
                self._qk_norm_cache[14]["k"],
            ) # torch.Size([1*5, 128, 448, 448])
            # out = torch.concat([self.hdr_conv(out + scene_feat), self.local_feat_conv(sensitivity)], dim=-3)
            out = torch.concat([self.hdr_conv(out + scene_feat), sensitivity], dim=-3)
            # scene_params = self.generate_tonemapper(hr_guide.view(b,v,*hr_guide.shape[1:]).detach(), rel_logt, out.view(b,v,*out.shape[1:]).detach())
            # tonemapping_param = self.tonemapping_param
            tonemapping_param = self.generate_tonemapper(hr_guide.view(b,v,*hr_guide.shape[1:]).detach(), rel_logt, out.view(b,v,*out.shape[1:]).detach())
            del scene_feat, sensitivity
            out = out.view(b, v, *out.shape[1:])

        del aggregated_tokens_list, patch_start_idx

        pts_flat = pts_all.flatten(2, 3) # torch.Size([1, 5, 448, 448, 3]) -> xxx
        scene_scale = pts_flat.norm(dim=-1).mean().clip(min=1e-8)

        if out.shape[-3] == self.raw_gs_dim + 1:
            anchor_feats, conf = out[:, :, : self.raw_gs_dim], out[:, :, self.raw_gs_dim]
        else:
            anchor_feats = torch.cat([out[:, :, :self.raw_gs_dim], out[:, :, self.raw_gs_dim+1:]], dim=-3)
            conf = out[:, :, self.raw_gs_dim]
        ...
        if self.cfg.voxelize:
            for b_i in range(b):
                ...
                neural_feats_list.append(neural_feats) # torch.Size([764543, 83]), [N,C]
                neural_pts_list.append(neural_pts) # torch.Size([764543, 3])
        else:
            for b_i in range(b):
        ...
        opacity = self.map_pdf_to_opacity(densities, global_step).squeeze(-1) # 让 opacity 在训练早期“没什么话语权”
        ...
        gaussians = self.gaussian_adapter.forward(
            neural_pts, # torch.Size([1, 764543, 3])
            depths, # torch.Size([1, 764543, 1])
            opacity, # torch.Size([1, 764543])
            neural_feats[..., 1:].squeeze(2), # torch.Size([1, 764543, 82])
        )
        if exposure != None:
            gaussians.tone_param = tonemapping_param.float()
            gaussians.mean_expos = mid_logt
            # gaussians.scene_params = scene_params.float()
