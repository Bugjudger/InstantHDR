...
class AnySplat(nn.Module, huggingface_hub.PyTorchModelHubMixin):
    def __init__(
        ...
        if encoder_cfg.pretrained_weights:
            self.load_weights(encoder_cfg.pretrained_weights)

    def load_weights(self, path):
        """Load an InstantHDR checkpoint or the original AnySplat initialization."""
        from pathlib import Path
        path = Path(path).expanduser()
        if not path.is_file():
            raise FileNotFoundError(f"Weights not found: {path}. See README.md.")
        if path.suffix == ".safetensors":
            from safetensors.torch import load_file
            # The initialization predates the HDR modules, which start randomly.
            missing, unexpected = self.load_state_dict(load_file(str(path)), strict=False)
            print(f"Initialization: {len(missing)} missing, {len(unexpected)} unexpected keys")
        else:
    ...
    def inference(self,
        ...
        encoder_output = self.encoder(context_image, exposure, global_step=0, visualization_dump=None)
    ...
    def forward(self, 
        context_image: torch.Tensor,
        exposure: list[torch.Tensor] = None,
        target_exposure: list[torch.Tensor] = None,
        global_step: int = 0,
        ...
        encoder_output = self.encoder(context_image, exposure, global_step, visualization_dump=visualization_dump)
        gaussians, pred_context_pose = encoder_output.gaussians, encoder_output.pred_context_pose
