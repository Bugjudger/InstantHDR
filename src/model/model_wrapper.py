...
from src.loss.loss_lpips import LossLpips
from src.loss.loss_mse import LossMse
...
class ModelWrapper(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        encoder_output, output = self.model(context_image, batch["context"]['exposure'], batch["context"]['exposure'], self.global_step, visualization_dump=visualization_dump)
        ...
        if (batch["context"]["hdr_image"]).mean() > 0:
            hdr_rgb_gt = tonemap_mu((batch["context"]["hdr_image"].float() / torch.quantile(batch["context"]["hdr_image"].float().flatten(-3), 0.95, dim=-1)[..., None, None, None]).clamp(0, 1))
            hdr2_psnr_probabilistic = compute_psnr(
                rearrange(hdr_rgb_gt, "b v c h w -> (b v) c h w"),
                rearrange(output.hdr_color, "b v c h w -> (b v) c h w"),
            )
            self.log("train/psnr_probabilistic-hdr2-splat", hdr2_psnr_probabilistic.mean())
        ...
        consis_absrel = abs_relative_difference(
            ...
            rearrange(depth_dict['conf_valid_mask'], "b v h w -> (b v) h w"),
        ...
        with torch.amp.autocast('cuda', enabled=False):
            ...
            # if (batch["context"]["hdr_image"]).mean() > 0:
            #     img = batch["context"]["hdr_image"].float()  # (B, V, C, H, W)
            #     scale = torch.quantile(
            #         img.reshape(*img.shape[:-3], -1),  # (B, V, C*H*W)
            #         0.95,
            #         dim=-1
            #     )[..., None, None, None]               # (B, V, 1, 1, 1)
            #     hdr_gt = tonemap_mu(img / scale.clamp(min=1e-6))
                
            #     for loss_fn in self.losses[:-1]:
            #         output.color = output.hdr_color
            #         batch["context"]["image"] = hdr_gt * 2 - 1
            #         loss = loss_fn.forward(output, batch, gaussians, depth_dict, self.global_step)
            #         self.log(f"loss/hdr2splat_{loss_fn.name}", loss)
            #         total_loss = total_loss + loss
        ...
        if self.test_cfg.align_pose:
            ...
            with self.benchmarker.time("decoder", num_calls=v):
                output = self.model.decoder.forward(
                    ...
                    batch["target"]["far"],
                    (h, w),
                    target_exposure=batch["target"]['exposure'],
                )
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):        
        ...
        # if _ == 6:
        #     batch["context"]["hdr_image"] = batch["context"]["image"][:, :, 3:, :, :]
        #     batch["context"]["image"] = batch["context"]["image"][:, :, :3, :, :]
        # else:
        #     batch["context"]["hdr_image"] = torch.tensor(-1.0)
        assert b == 1
        visualization_dump = {}

        encoder_output, output = self.model((batch["context"]["image"] + 1) / 2, batch["context"]['exposure'], batch["context"]['exposure'], self.global_step, visualization_dump=visualization_dump)
        gaussians, pred_pose_enc_list, depth_dict = encoder_output.gaussians, encoder_output.pred_pose_enc_list, encoder_output.depth_dict
        ...
        # Compute validation metrics.
        hdr_rgb_pred, hdr_rgb_gt = None, None
        if (batch["context"]["hdr_image"]).mean() > 0:
            hdr_rgb_pred = output.hdr_color[0].float()
            hdr_rgb_gt = tonemap_mu((batch["context"]["hdr_image"][0].float() / torch.quantile(batch["context"]["hdr_image"][0].float().flatten(-3), 0.95, dim=-1)[..., None, None, None]).clamp(0, 1))

            psnr = compute_psnr(hdr_rgb_gt, hdr_rgb_pred).mean()
            self.log(f"val/hdr_psnr", psnr)
            lpips = compute_lpips(hdr_rgb_gt, hdr_rgb_pred).mean()
            self.log(f"val/hdr_lpips", lpips)
            ssim = compute_ssim(hdr_rgb_gt, hdr_rgb_pred).mean()
            self.log(f"val/hdr_ssim", ssim)
        ...
        self.log("val/consis_mse", diff_map[depth_dict['conf_valid_mask']].mean())
        ...
        if hdr_rgb_pred is None:
            ...
            comparison = hcat(
                ...
                add_label(vcat(*depth_pred), "Depth (Prediction)"),
                add_label(vcat(*model_depth_pred), "Depth (VGGT Prediction)"),
                add_label(vcat(*hdr_rgb_gt), "HDR (Ground Truth)"),
                # add_label(vcat(*encoder_output.log_hdr[0].float()), "HDR1 (Prediction)"),
                add_label(vcat(*hdr_rgb_pred), "HDR2 (Prediction)"),
                add_label(vcat(*colored_diff_map), "Diff Map"),
            )
    ...
    def render_video_generic(
        ...
        encoder_output = self.model.encoder((batch["context"]["image"]+1)/2, batch["context"]["exposure"], self.global_step)
        ...
        output = self.model.decoder.forward(
            ...
            target_exposure= [batch["context"]["exposure"][0]] * num_frames
        )
        images = [
            vcat(rgb, depth)
